...
MelBanks::MelBanks(const MelBanksOptions &opts,
  ...
  float ww[23] = {3.262, 8.877, 5.41, 1.867, 0.758, 0.268, 0.085, 0.002, 0.004, 0.001, 0.0, 0.0, 0.003, 0.017, 0.03, 0.176, 0.355, 0.487, 0.521, 0.402, 0.318, 0.141, 0.005};
  ...
  for (int32 bin = 0; bin < num_bins; bin++) {
    ...
    for (int32 i = 0; i < num_fft_bins; i++) {
      ...
      if (mel > left_mel && mel < right_mel) {
        BaseFloat weight;
        if (mel <= center_mel)
          weight = ww[bin] * (mel - left_mel) / (center_mel - left_mel);
          //weight = (mel - left_mel) / (center_mel - left_mel);
        else
          weight = ww[bin] * (right_mel-mel) / (right_mel-center_mel);
