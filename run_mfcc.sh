...
steps/make_mfcc.sh --mfcc-config conf/$config --nj 20 --cmd "$train_cmd" \
    $dpath/UBM log/_log_mfcc $mfccdir
steps/make_mfcc.sh --mfcc-config conf/$config --nj 20 --cmd "$train_cmd" \
    $dpath/Dev log/_log_mfcc $mfccdir
...
for phone in 1 2 3 4 5 6 ;do
	...
	mfccdir=`pwd`/_mfcc_${phone}
	vaddir=`pwd`/_mfcc_${phone}

	for data in model raw replay; do
		steps/make_mfcc.sh --mfcc-config conf/$config --nj 10 --cmd "$train_cmd" $rpath/$phone/$data log/_log_mfcc $mfccdir
...
if $cleanup; then
  ...
  rm -r _mfcc* log score/total/* exp_novad
